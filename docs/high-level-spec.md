...
- **Perspective camera (free-look).** Add a perspective projection mode alongside the iso camera. Motivation: scene readability at varying tree/building scale was a noted pain point during M7.2 — iso squashes everything onto the same plane and makes judgment of relative object size visually unreliable. A perspective camera plus mouse-look would give a faithful "walkthrough" view for demos and debugging, while iso remains the default gameplay projection. Touches: `game::camera` (add a `CameraMode` switch), `voxel::raymarch` (per-pixel ray direction becomes pos-dependent), `render::composite` (may bypass the holder-sprite composite in favor of direct raymarch into the backbuffer for perspective mode).
